...
class RcsMessageTypeAnalyticsPage(BasePage):

    REPORT_URL = "/rcs/analytics/message-type"
    TABLE_NAME = "rcs_message_type_report"

    # Confirmed column order from the live <thead> (table-head-0 .. -16)
    COLUMN_INDEX = {
        "duration": 0, "product": 1, "agent": 2, "message_type": 3,
    ...
    # ── Page header ───────────────────────────────────────────────────────────
    PAGE_TITLE = "xpath=//h1[contains(normalize-space(.),'RCS Message Type Analytics')]"

    # ── Top filter bar ───────────────────────────────────────────────────────
    DATE_RANGE_PICKER = "#rcs-message-type-date-range-picker"
    REPORT_TYPE_SELECT = "#rcs-message-type-report-type-select"
    GROUP_BY_LABEL = "#rcs-message-type-dimension-multiselect-label"
    GROUP_BY_BUTTON = "xpath=//span[@id='rcs-message-type-dimension-multiselect-label']/ancestor::button[1]"
    GROUP_BY_DROPDOWN = "#rcs-message-type-dimension-multiselect-dropdown"

    SEARCH_BOX = "input[wire\\:model\\.live='search'][placeholder='Search by Message Type']"
    ...
    def is_report_page(self):
        ...
        return "/rcs/analytics/message-type" in url and "login" not in url.lower()
    ...
    def open_filters_popover(self):
        ...
        # instead of a fixed page.wait_for_timeout(500) -- a real 12-worker
        # parallel run showed toggle_product_filter_option()'s checkbox
        # wait (input[id^='...-filter-product-'][value='transactional'])
        # time out even though the click on FILTERS_BUTTON had registered:
        # under load the panel's own open animation/render can outlast a
        # fixed 500ms guess, so a caller waiting on a control INSIDE the
        # panel starts its own wait before the panel is actually there.
        # FILTER_AGENT_INPUT is the same element the guard above already
        # treats as "the panel is open", so wait on it for real instead of
        # guessing a delay.
        self.h.wait_for_element_visible(self.FILTER_AGENT_INPUT, timeout=10000)
    ...
    def toggle_group_by_dimension(self, dim):
        ...
        PRE-CHECKED by default on page load."""
    ...
    def click_export_csv(self, timeout_ms=30000):
        ...
        try:
            ...
            filename = download.suggested_filename or "rcs_message_type_report_export.csv"
