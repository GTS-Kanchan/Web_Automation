...
class RcsStatusAnalyticsPage(BasePage):

    REPORT_URL = "/rcs/analytics/status"
    TABLE_NAME = "rcs_status_report"

    # Confirmed column order from the live <thead> (table-head-0 .. -10).
    # NOTE: this report has NO sent/delivered/failed/rejected/dlr-awaited
    # breakdown and NO campaign-name/template-name column — CONFIRMED
    # absent from wire:snapshot selectableColumns (row dimension IS status).
    COLUMN_INDEX = {
        "duration": 0, "product": 1, "agent": 2, "status": 3,
        "total_count": 4, "interactions": 5, "quick_reply_total": 6,
        "quick_reply_unique": 7, "cta_total_clicks": 8,
        "cta_unique_clicks": 9, "total_charges": 10,
    }

    # ── Page header ───────────────────────────────────────────────────────────
    PAGE_TITLE = "xpath=//h1[contains(normalize-space(.),'RCS Status Analytics')]"

    # ── Top filter bar ───────────────────────────────────────────────────────
    DATE_RANGE_PICKER = "#rcs-status-date-range-picker"
    REPORT_TYPE_SELECT = "#rcs-status-report-type-select"
    GROUP_BY_LABEL = "#rcs-status-dimension-multiselect-label"
    GROUP_BY_BUTTON = "xpath=//span[@id='rcs-status-dimension-multiselect-label']/ancestor::button[1]"
    GROUP_BY_DROPDOWN = "#rcs-status-dimension-multiselect-dropdown"
    ...
    def is_report_page(self):
        ...
        return "/rcs/analytics/status" in url and "login" not in url.lower()
    ...
    def select_report_type(self, value):
        """value: 'daily' | 'weekly' | 'monthly'. NOTE: unlike SMS's Status
        Report, this RCS report defaults to 'daily' and only has these
        three options — CONFIRMED live DOM x-data reportType: 'daily'."""
    ...
    def toggle_group_by_dimension(self, dim):
        ...
        THREE dimensions here — no 'source'."""
    ...
    def uncheck_first_optional_column(self):
        ...
        NOTE: this report only has 7 toggleable columns (total-count,
        interactions, quick-reply-total, quick-reply-unique,
        cta-total-clicks, cta-unique-clicks, total-charges) — CONFIRMED
        absent is any sent/delivered/failed/rejected/dlr-awaited breakdown
    ...
    def click_export_csv(self, timeout_ms=30000):
        ...
        try:
            ...
            filename = download.suggested_filename or "rcs_status_report_export.csv"
