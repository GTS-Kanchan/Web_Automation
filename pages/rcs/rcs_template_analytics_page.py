...
class RcsTemplateAnalyticsPage(BasePage):

    REPORT_URL = "/rcs/analytics/template"
    TABLE_NAME = "rcs_template_report"

    # Confirmed column order from the live <thead> (table-head-0 .. -16)
    COLUMN_INDEX = {
        "duration": 0, "product": 1, "agent": 2, "template_name": 3,
        "total_count": 4, "sent_count": 5, "delivered_count": 6,
        "read_count": 7, "failed_count": 8, "rejected_count": 9,
        "dlr_awaited_count": 10, "interactions": 11, "quick_reply_total": 12,
        "quick_reply_unique": 13, "cta_total_clicks": 14,
        "cta_unique_clicks": 15, "total_charges": 16,
    }

    # ── Page header ───────────────────────────────────────────────────────────
    PAGE_TITLE = "xpath=//h1[contains(normalize-space(.),'RCS Template Analytics')]"

    # ── Top filter bar ───────────────────────────────────────────────────────
    DATE_RANGE_PICKER = "#rcs-template-date-range-picker"
    # CONFIRMED live DOM: no "-template-" segment in this one id, unlike every
    # other control on this page and every other RCS Analytics report.
    REPORT_TYPE_SELECT = "#rcs-report-type-select"
    GROUP_BY_LABEL = "#rcs-template-dimension-multiselect-label"
    GROUP_BY_BUTTON = "xpath=//span[@id='rcs-template-dimension-multiselect-label']/ancestor::button[1]"
    GROUP_BY_DROPDOWN = "#rcs-template-dimension-multiselect-dropdown"
    ...
    FILTER_AGENT_INPUT = ".async-select-filter-agent input[placeholder='Search Agent']"
    ...
    def is_report_page(self):
        ...
        return "/rcs/analytics/template" in url and "login" not in url.lower()
    ...
    def get_row_count(self):
        # Delegates to BasePage._count_data_rows() — see that method's
        # docstring for why the retry loop is kept even though Playwright's
        # auto-re-resolving Locator API structurally eliminates the
        # StaleElementReferenceException race that motivated it in Selenium.
    ...
    def get_column_values(self, column_name):
        """Return the text of every row's cell for `column_name`.

        Gives the table a brief chance to populate before giving up
        empty-handed, to avoid a Livewire-render timing race — same class
        of fix already applied to _count_data_rows / _get_headers_safe in
        base_page.py."""
        ...
        while True:
            ...
            if values or time.time() > end_time:
                return values
            self.page.wait_for_timeout(500)
    ...
    def _select_first_async_option(self, container_css):
        ...
        every async-select field built in this project: the listbox
        container (div[x-ref='options'][role='listbox']) IS confirmed, but
        no populated option-item markup was captured (only the empty 'No
        results found' state) — returns False if none match, rather than
        guessing further."""
        try:
    ...
    def filter_by_agent(self, text):
        ...
        box.type(text)
        self.page.wait_for_timeout(1000)
        return self._select_first_async_option(".async-select-filter-agent")
    ...
    def select_product_filter(self, value):
        ...
        CONFIRMED live DOM: Product here is a plain <select> with exactly
        these three named options (no 'Multi-use')."""
        self.open_filters_popover()
    ...
    def toggle_group_by_dimension(self, dim):
        ...
        THREE dimensions here — no 'source'. Also note 'product' is
        PRE-CHECKED by default on page load (CONFIRMED wire:snapshot
        dimensions:["product"])."""
    ...
    def select_date_range(self, from_day_offset=10, to_day_offset=3):
        ...
        click — Playwright's Locator API re-resolves on every call, so the
        StaleElementReferenceException retry the Selenium version needed
        after flatpickr redraws the grid is structurally unnecessary here,
        but the offset-from-end indexing is kept identical for parity.
    ...
    def click_export_csv(self, timeout_ms=30000):
        ...
        try:
            ...
            filename = download.suggested_filename or "rcs_template_report_export.csv"
