...
class RcsCountryAnalyticsPage(BasePage):

    REPORT_URL = "/rcs/analytics/country"
    TABLE_NAME = "rcs_country_report"

    # Confirmed column order from the live <thead> (table-head-0 .. -16)
    COLUMN_INDEX = {
        "duration": 0, "product": 1, "agent": 2, "country_code": 3,
    ...
    # ── Page header ───────────────────────────────────────────────────────────
    PAGE_TITLE = "xpath=//h1[contains(normalize-space(.),'RCS Country Analytics')]"

    # ── Top filter bar ───────────────────────────────────────────────────────
    DATE_RANGE_PICKER = "#rcs-country-date-range-picker"
    REPORT_TYPE_SELECT = "#rcs-country-report-type-select"
    GROUP_BY_LABEL = "#rcs-country-dimension-multiselect-label"
    GROUP_BY_BUTTON = "xpath=//span[@id='rcs-country-dimension-multiselect-label']/ancestor::button[1]"
    GROUP_BY_DROPDOWN = "#rcs-country-dimension-multiselect-dropdown"
    ...
    def is_report_page(self):
        ...
        return "/rcs/analytics/country" in url and "login" not in url.lower()
    ...
    def click_export_csv(self, timeout_ms=30000):
        ...
        try:
            ...
            filename = download.suggested_filename or "rcs_country_report_export.csv"
