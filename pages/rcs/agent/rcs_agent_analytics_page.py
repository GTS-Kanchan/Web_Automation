...
class RcsAgentAnalyticsPage(BasePage):
    ...
    REPORT_URL = "/rcs/analytics/agent"
    TABLE_NAME = "rcs_agent_report"

    # Confirmed column order from the live <thead> (table-head-0 .. -15)
    COLUMN_INDEX = {
        "duration": 0, "product": 1, "agent": 2,
        "total_count": 3, "sent_count": 4, "delivered_count": 5,
        "read_count": 6, "failed_count": 7, "rejected_count": 8,
        "dlr_awaited_count": 9, "interactions": 10, "quick_reply_total": 11,
        "quick_reply_unique": 12, "cta_total_clicks": 13,
        "cta_unique_clicks": 14, "total_charges": 15,
    }

    # ── Page header ───────────────────────────────────────────────────────────
    PAGE_TITLE = "xpath=//h1[contains(normalize-space(.),'RCS Agent Analytics')]"

    # ── Top filter bar ───────────────────────────────────────────────────────
    DATE_RANGE_PICKER = "#rcs-agent-date-range-picker"
    REPORT_TYPE_SELECT = "#rcs-agent-report-type-select"
    GROUP_BY_LABEL = "#rcs-agent-dimension-multiselect-label"
    GROUP_BY_BUTTON = "xpath=//span[@id='rcs-agent-dimension-multiselect-label']/ancestor::button[1]"
    GROUP_BY_DROPDOWN = "#rcs-agent-dimension-multiselect-dropdown"

    SEARCH_BOX = "input[wire\\:model\\.live='search'][placeholder='Search by Agent']"

    # ── Filters popover (NO Agent filter — Agent IS the row dimension) ─────────
    ...
    def is_report_page(self):
        ...
        return "/rcs/analytics/agent" in url and "login" not in url.lower()
    ...
    def open_filters_popover(self):
        ...
        # FILTER_DEPARTMENT_INPUT is the same element the guard above
        # already treats as "the panel is open", so wait on it for real
