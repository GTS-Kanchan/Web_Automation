...
class WhatsappTemplateAnalyticsPage(BasePage):
    ...
    REPORT_URL = "/whatsapp/analytics/template"
    TABLE_NAME = "whatsapp_template_report"

    COLUMN_INDEX = {
        "duration": 0, "product": 1, "waba_number": 2, "template": 3,
    ...
    PAGE_TITLE = "xpath=//h1[contains(normalize-space(.),'Template Analytics')]"

    DATE_RANGE_PICKER = "#whatsapp-template-date-range-picker"
    REPORT_TYPE_SELECT = "#whatsapp-template-report-type-select"
    GROUP_BY_LABEL = "#whatsapp-template-dimension-multiselect-label"
    GROUP_BY_BUTTON = (
        "xpath=//span[@id='whatsapp-template-dimension-multiselect-label']/ancestor::button[1]"
    )
    GROUP_BY_DROPDOWN = "#whatsapp-template-dimension-multiselect-dropdown"
    ...
    def is_report_page(self):
        ...
        return "/whatsapp/analytics/template" in url and "login" not in url.lower()
    ...
    def get_date_range_value(self):
        ...
        # input_value() reads the live property instead -- the same pattern
        # already used correctly by get_product_filter_value()/
        # get_report_type() elsewhere in this file.
