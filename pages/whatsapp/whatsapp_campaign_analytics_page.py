...
class WhatsappCampaignAnalyticsPage(BasePage):

    REPORT_URL = "/whatsapp/analytics/campaign"
    TABLE_NAME = "whatsapp_campaign_report"

    # NOTE: table-head-0 is a decorative "Action" column (CONFIRMED live DOM:
    # every sampled row's <td> for this column contained only Alpine
    # if/endif comment-block directives, no button/svg/anchor content at
    # all) — deliberately excluded from COLUMN_INDEX and from
    # get_column_values() lookups; no click-interaction test is built for
    # it. All other indices below are shifted +1 to account for it.
    COLUMN_INDEX = {
        "duration": 1, "product": 2, "waba_number": 3, "campaign": 4,
        "template": 5, "total": 6, "sent": 7, "delivered": 8, "read": 9,
        "failed": 10, "rejected": 11, "dlr_awaited": 12, "interactions": 13,
        "quick_reply_total": 14, "quick_reply_unique": 15,
        "cta_total_clicks": 16, "cta_unique_clicks": 17, "total_charges": 18,
    }

    PAGE_TITLE = "xpath=//h1[contains(normalize-space(.),'Campaign Analytics')]"

    DATE_RANGE_PICKER = "#whatsapp-campaign-date-range-picker"
    REPORT_TYPE_SELECT = "#whatsapp-campaign-report-type-select"
    GROUP_BY_LABEL = "#whatsapp-campaign-dimension-multiselect-label"
    GROUP_BY_BUTTON = (
        "xpath=//span[@id='whatsapp-campaign-dimension-multiselect-label']/ancestor::button[1]"
    )
    GROUP_BY_DROPDOWN = "#whatsapp-campaign-dimension-multiselect-dropdown"
    ...
    def is_report_page(self):
        ...
        return "/whatsapp/analytics/campaign" in url and "login" not in url.lower()
    ...
    def get_row_count(self):
        # Delegates to BasePage._count_data_rows(), which retries on
        # transient re-render races — CONFIRMED necessary via a real pytest
        # run in this project (WhatsApp Usage Analytics TC11: a Livewire
        # re-render mid-iteration, triggered by clear_search() right
        # after an invalid search, raised a StaleElementReferenceException
        # in the original Selenium suite). Same fix applied across every
        # RCS/WhatsApp Analytics page in this project, since they all share
        # this identical row-counting pattern and are equally exposed to
        # the same Livewire re-render race.
