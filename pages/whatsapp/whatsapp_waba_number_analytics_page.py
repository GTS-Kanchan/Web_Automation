...
class WhatsappWabaNumberAnalyticsPage(BasePage):

    REPORT_URL = "/whatsapp/analytics/waba-number"
    TABLE_NAME = "whatsapp_waba_number_report"

    COLUMN_INDEX = {
        "duration": 0, "product": 1, "waba_number": 2,
        "total": 3, "sent": 4, "delivered": 5, "read": 6, "failed": 7,
        "rejected": 8, "dlr_awaited": 9, "interactions": 10,
        "quick_reply_total": 11, "quick_reply_unique": 12,
        "cta_total_clicks": 13, "cta_unique_clicks": 14, "total_charges": 15,
    }

    PAGE_TITLE = "xpath=//h1[contains(normalize-space(.),'WABA Number Analytics')]"

    DATE_RANGE_PICKER = "#whatsapp-waba-number-date-range-picker"
    REPORT_TYPE_SELECT = "#whatsapp-waba-number-report-type-select"
    GROUP_BY_LABEL = "#whatsapp-waba-number-dimension-multiselect-label"
    GROUP_BY_BUTTON = (
        "xpath=//span[@id='whatsapp-waba-number-dimension-multiselect-label']/ancestor::button[1]"
    )
    GROUP_BY_DROPDOWN = "#whatsapp-waba-number-dimension-multiselect-dropdown"

    SEARCH_BOX = "input[wire\\:model\\.live='search'][placeholder='Search by WABA Number']"
    ...
    def is_report_page(self):
        ...
        return "/whatsapp/analytics/waba-number" in url and "login" not in url.lower()
