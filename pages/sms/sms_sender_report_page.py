...
class SmsSenderReportPage(BasePage):

    REPORT_URL = "/channels/sms/reports/sender"
    TABLE_NAME = "sms_sender_report"

    # Confirmed column order from the live <thead> (table-head-0 .. -19)
    COLUMN_INDEX = {
        "duration": 0, "sender": 1, "product": 2, "total_count": 3,
    ...
    # ── Page header ───────────────────────────────────────────────────────────
    PAGE_TITLE = "xpath=//h1[contains(normalize-space(.),'Sender Report')]"

    # ── Top filter bar ───────────────────────────────────────────────────────
    DATE_RANGE_PICKER = "#sender-date-range-picker"
    REPORT_TYPE_SELECT = "#report-type-select"
    GROUP_BY_LABEL = "#sender-dimension-multiselect-label"
    GROUP_BY_BUTTON = "xpath=//span[@id='sender-dimension-multiselect-label']/ancestor::button[1]"
    GROUP_BY_DROPDOWN = "#sender-dimension-multiselect-dropdown"

    SEARCH_BOX = "input[wire\\:model\\.live='search'][placeholder='Search by Sender']"
    ...
    def is_report_page(self):
        ...
        return "/channels/sms/reports/sender" in url and "login" not in url.lower()
    ...
    def click_export_csv(self, timeout_ms=30000):
        ...
        try:
            ...
            filename = download.suggested_filename or "sms_sender_report_export.csv"
