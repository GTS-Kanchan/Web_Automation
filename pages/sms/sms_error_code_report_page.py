...
class SmsErrorCodeReportPage(BasePage):

    REPORT_URL = "/channels/sms/reports/error-code"
    TABLE_NAME = "sms_error_code_report"

    # Confirmed column order from the live <thead> (table-head-0 .. -6).
    COLUMN_INDEX = {
        "duration": 0, "product": 1, "error_code": 2, "error_description": 3,
        "total_count": 4, "total_units": 5, "percentage_share": 6,
    }

    # ── Page header ───────────────────────────────────────────────────────────
    PAGE_TITLE = "xpath=//h1[contains(normalize-space(.),'Error Code Report')]"

    # ── Top filter bar ───────────────────────────────────────────────────────
    DATE_RANGE_PICKER = "#error-code-date-range-picker"
    # NOTE: unique id on this report — every other SMS report suite uses
    # the generic id "report-type-select". CONFIRMED live DOM.
    REPORT_TYPE_SELECT = "#error-code-report-type-select"
    GROUP_BY_LABEL = "#error-code-dimension-multiselect-label"
    GROUP_BY_BUTTON = "xpath=//span[@id='error-code-dimension-multiselect-label']/ancestor::button[1]"
    GROUP_BY_DROPDOWN = "#error-code-dimension-multiselect-dropdown"

    SEARCH_BOX = "input[wire\\:model\\.live='search'][placeholder='Search by error/operator']"

    # ── Filters popover ──────────────────────────────────────────────────────
    # NOTE: no Department or User filter on this report — CONFIRMED
    # absent from the live DOM (only Product select + Source
    ...
    FILTER_SOURCE_CHECKBOXES = (
        f"#{TABLE_NAME}-filter-source-wrapper input[type='checkbox']"
        f":not(#{TABLE_NAME}-filter-source-select-all)"
    )
    ...
    # NOTE: this report's live DOM was captured on page 1, where the
    # rappasoft-tables Previous control renders as a static disabled
    # <span>, not a clickable element — so its enabled wire:click markup
    # was never directly observed. "previousPage(table)" is inferred from
    # the rappasoft/laravel-livewire-tables package's own documented
    # public API. Treat interactions using this locator as best-effort.
    PREV_PAGE_BTN = f"xpath=//button[contains(@*[name()='wire:click'],\"previousPage('{TABLE_NAME}')\")]"
    ...
    def is_report_page(self):
        ...
        return "/channels/sms/reports/error-code" in url and "login" not in url.lower()
    ...
    def get_all_row_data(self):
        """Returns a list of dicts, one per visible row, keyed by
        COLUMN_INDEX names. Used for formula/grouping validation (e.g.
        Percentage Share) without repeated per-column DOM queries."""
        rows = self.page.locator(self.TABLE_ROWS)
        result = []
        for i in range(rows.count()):
            tds = rows.nth(i).locator("td")
            n = tds.count()
            if n < len(self.COLUMN_INDEX):
                continue
            texts = [t.strip() for t in tds.all_inner_texts()]
            if not any(texts):
                continue
            entry = {name: texts[idx] for name, idx in self.COLUMN_INDEX.items()}
            result.append(entry)
        return result
    ...
    def select_report_type(self, value):
        """value: 'daily' | 'weekly' | 'monthly'. NOTE: this report has NO
        'hourly' option, unlike Status/Latency Report — CONFIRMED live
        DOM (only 3 <option> elements)."""
    ...
    def toggle_group_by_dimension(self, dim):
        """dim: 'product' | 'source' | 'operator' (3 dims, unique to this
        report — "operator" is not a dimension on any other SMS report).
        Defaults to "product" pre-checked ("1 selected") — CONFIRMED live
        DOM."""
    ...
    def uncheck_first_optional_column(self):
        ...
        'total-count') on success, or None if there was nothing to
        uncheck. NOTE: this report only has 3 toggleable columns
        (total-count, total-units, percentage-share)."""
    ...
    def click_prev_page(self):
        ...
        wire:click markup was never directly observed in the captured DOM
        (only the disabled page-1 state was). Returns False instead of
        raising if the button isn't found/clickable."""
        try:
            self._js_click(self.PREV_PAGE_BTN, timeout=5000)
            self.page.wait_for_timeout(1500)
            return True
        except Exception:
            return False
