...
class SmsStatusReportPage(BasePage):

    REPORT_URL = "/channels/sms/reports/status"
    TABLE_NAME = "sms_status_report"

    # Confirmed column order from the live <thead> (table-head-0 .. -6).
    # NOTE: this report has NO submitted/delivered/failed/rejected/
    # dlr-awaited or delivery-charges/delivery-surcharge/delivery(%)
    # columns — CONFIRMED absent from wire:snapshot selectableColumns.
    COLUMN_INDEX = {
        "duration": 0, "status": 1, "product": 2, "total_count": 3,
        "total_units": 4, "total_charges": 5, "surcharge": 6,
    }

    # ── Page header ───────────────────────────────────────────────────────────
    PAGE_TITLE = "xpath=//h1[contains(normalize-space(.),'Status Report')]"

    # ── Top filter bar ───────────────────────────────────────────────────────
    DATE_RANGE_PICKER = "#status-date-range-picker"
    REPORT_TYPE_SELECT = "#report-type-select"
    GROUP_BY_LABEL = "#status-dimension-multiselect-label"
    GROUP_BY_BUTTON = "xpath=//span[@id='status-dimension-multiselect-label']/ancestor::button[1]"
    GROUP_BY_DROPDOWN = "#status-dimension-multiselect-dropdown"
    ...
    def is_report_page(self):
        ...
        return "/channels/sms/reports/status" in url and "login" not in url.lower()
    ...
    def select_report_type(self, value):
        """value: 'hourly' | 'daily' | 'weekly' | 'monthly'. NOTE: unlike
        every other SMS report, this report defaults to 'hourly' (not
        'daily') — CONFIRMED live DOM x-data reportType: 'hourly'."""
    ...
    def uncheck_first_optional_column(self):
        ...
        uncheck. NOTE: this report only has 4 toggleable columns
        (total-count, total-units, total-charges, surcharge) — CONFIRMED
        absent are the submitted/delivered/failed/rejected/dlr-awaited/
        delivery-charges/delivery-surcharge/delivery(%) columns present
        on other reports."""
    ...
    def click_export_csv(self, timeout_ms=30000):
        ...
        try:
            ...
            filename = download.suggested_filename or "sms_status_report_export.csv"
