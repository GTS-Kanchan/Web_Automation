...
class SmsUsageReportPage(BasePage):
    ...
    def click_export_csv(self, timeout_ms=30000):
        ...
        capturing anything -- same broken pattern originally found on the
        Template/Messages export methods).

        Returns {"elapsed_s", "file_path", "file_size"} on success, or
        None on failure (timeout / no download triggered) -- kept as a
        never-raises contract so the existing test_usage_report_TC14
        caller, which discards the return value, is unaffected.
        """
        try:
            ...
            filename = download.suggested_filename or "sms_usage_report_export.csv"
