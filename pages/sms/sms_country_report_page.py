...
class SmsCountryReportPage(BasePage):

    REPORT_URL = "/channels/sms/reports/country"
    TABLE_NAME = "sms_country_report"

    # Confirmed column order from the live <thead> (table-head-0 .. -19)
    COLUMN_INDEX = {
        "duration": 0, "country_code": 1, "product": 2, "total_count": 3,
        "submitted_count": 4, "delivered_count": 5, "failed_count": 6,
        "rejected_count": 7, "dlr_awaited_count": 8, "total_units": 9,
        "submitted_units": 10, "delivered_units": 11, "failed_units": 12,
        "rejected_units": 13, "dlr_awaited_units": 14, "total_charges": 15,
        "delivery_charges": 16, "surcharge": 17, "delivery_surcharge": 18,
        "delivery_pct": 19,
    }

    # ── Page header ───────────────────────────────────────────────────────────
    PAGE_TITLE = "xpath=//h1[contains(normalize-space(.),'Country Report')]"

    # ── Top filter bar ───────────────────────────────────────────────────────
    DATE_RANGE_PICKER = "#country-date-range-picker"
    REPORT_TYPE_SELECT = "#report-type-select"
    GROUP_BY_LABEL = "#country-dimension-multiselect-label"
    GROUP_BY_BUTTON = "xpath=//span[@id='country-dimension-multiselect-label']/ancestor::button[1]"
    GROUP_BY_DROPDOWN = "#country-dimension-multiselect-dropdown"
    ...
    # use @*[name()='wire:key'] / @*[name()='wire:click'] instead
    # everywhere below.
    COLUMN_CHECKBOXES = (
    ...
    def is_report_page(self):
        ...
        return "/channels/sms/reports/country" in url and "login" not in url.lower()
    ...
    def toggle_group_by_dimension(self, dim):
        """dim: 'product' | 'department' | 'user'. NOTE: this report
        defaults to "product" already checked (initialSelected=
        ["product"]), so the label starts at "1 selected" rather than
        "Select Dimensions" — CONFIRMED live DOM."""
    ...
    def select_date_range(self, from_day_offset=10, to_day_offset=3):
        ...
        this project. Cells are restricted to same-month-only and
        re-queried fresh before each click — Playwright's Locator API
        re-resolves on every call, so the StaleElementReferenceException
        retry the Selenium version needed after flatpickr redraws the grid
    ...
    def check_column(self, value):
        ...
        Same sessionStorage-persistence gotcha as every other report in
        this project — column visibility survives a plain
        navigate_to_report(), so this restores state after
        uncheck_first_optional_column() in tests."""
        self.open_columns_dropdown()
    ...
    def click_export_csv(self, timeout_ms=30000):
        ...
        try:
            ...
            filename = download.suggested_filename or "sms_country_report_export.csv"
