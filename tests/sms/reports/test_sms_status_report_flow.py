...
def test_tc15_export_csv(status_report_page):
    """TC_15: Export CSV button triggers a download, and the downloaded
    file's header row exactly matches the defined SMS Status Report export
    specification (constants/sms_status_report_headers.py). Reuses the
    same generic utils/file_validator.py used by every other SMS export's
    header validation; only the expected-header list differs.
    """
    ensure_on_report_page(status_report_page)
    ...
    print("[DOWNLOAD] SMS Status Report export file downloaded")
    print(f"[DOWNLOAD] File: {os.path.basename(file_path)}")

    print("[VALIDATION] Reading file headers")
    print(f"[VALIDATION] Expected headers: {len(EXPECTED_SMS_STATUS_REPORT_HEADERS)}")
    print(f"[VALIDATION] Expected header names: {EXPECTED_SMS_STATUS_REPORT_HEADERS}")
    try:
        actual_headers = validate_file_headers(file_path, EXPECTED_SMS_STATUS_REPORT_HEADERS)
    except FileNotDownloadedError as exc:
        pytest.fail(f"[DOWNLOAD] {exc}")
    except (UnsupportedFileTypeError, EmptyFileError) as exc:
        print("[VALIDATION] SMS Status Report header validation: FAIL")
        pytest.fail(str(exc))
    ...
    print("[VALIDATION] SMS Status Report header validation: PASS")
