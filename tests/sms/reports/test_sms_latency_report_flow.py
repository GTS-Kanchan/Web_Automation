...
def test_tc12_export_latency_report_csv(latency_report_page):
    """TC_12: Export CSV button triggers a download, and the downloaded
    file's header row exactly matches the defined SMS Latency Report
    export specification (constants/sms_latency_report_headers.py).
    Reuses the same generic utils/file_validator.py used by every other
    SMS export's header validation; only the expected-header list differs.
    """
    ensure_on_report_page(latency_report_page)
    ...
    print("[DOWNLOAD] SMS Latency Report export file downloaded")
    print(f"[DOWNLOAD] File: {os.path.basename(file_path)}")

    print("[VALIDATION] Reading file headers")
    print(f"[VALIDATION] Expected headers: {len(EXPECTED_SMS_LATENCY_REPORT_HEADERS)}")
    print(f"[VALIDATION] Expected header names: {EXPECTED_SMS_LATENCY_REPORT_HEADERS}")
    try:
        actual_headers = validate_file_headers(file_path, EXPECTED_SMS_LATENCY_REPORT_HEADERS)
    except FileNotDownloadedError as exc:
        pytest.fail(f"[DOWNLOAD] {exc}")
    except (UnsupportedFileTypeError, EmptyFileError) as exc:
        print("[VALIDATION] SMS Latency Report header validation: FAIL")
        pytest.fail(str(exc))
    ...
    print("[VALIDATION] SMS Latency Report header validation: PASS")
...
# the project owner (the SMS Latency page's default on-screen table columns),
# not a guess and not the CSV/Excel export header list above (which is a
# separate, already-existing concept). See
# constants/sms_latency_report_ui_headers.py for the full header list and provenance note.
# ══════════════════════════════════════════════════════════════════════════════

@pytest.mark.smoke
def test_ui_default_table_headers_full(latency_report_page):
    """All confirmed default on-screen table column headers for the SMS Latency
