"""
WhatsApp Campaign Analytics — Single Sequential Flow
=====================================================
Covers: Channels → WhatsApp → Analytics → Campaign page
(URL: /whatsapp/analytics/campaign; breadcrumb reads "WhatsApp Campaign
Analytics" but this page's own <h1> reads plain "Campaign Analytics" —
...
Key confirmed structural facts that shaped this suite:
  - 19 columns total — the MOST of any report in this project: Action
    (table-head-0, CONFIRMED decorative — every sampled row's <td>
    contained only Alpine if/endif comment-block directives, no
    button/svg/anchor markup at all; deliberately no click-interaction test
    built for it), duration/product/waba-number/campaign/template (5 fixed
  ...
  - Search placeholder is "Search by Campaign".
  - CONFIRMED live pagination total at capture time: 258 results.
...
Run:
    pytest tests/test_whatsapp_campaign_analytics_flow.py -v
"""
import pytest

from pages.whatsapp.whatsapp_campaign_analytics_page import WhatsappCampaignAnalyticsPage
...
def campaign_analytics_page(module_logged_in_page):
    p = WhatsappCampaignAnalyticsPage(module_logged_in_page)
...
def test_campaign_analytics_TC01_page_loads(campaign_analytics_page):
    """TC_01: WhatsApp Campaign Analytics page loads successfully without
    errors."""
    ensure_on_report_page(campaign_analytics_page)
    assert campaign_analytics_page.is_report_page(), "URL should contain /whatsapp/analytics/campaign"
...
def test_campaign_analytics_title_displayed(campaign_analytics_page):
    """Page title 'Campaign Analytics' is visible (CONFIRMED live DOM — this
    report's own <h1> omits the "WhatsApp" prefix seen in the breadcrumb)."""
    ensure_on_report_page(campaign_analytics_page)
    assert "Campaign Analytics" in campaign_analytics_page.get_page_title_text()
...
def test_campaign_analytics_TC04_valid_custom_date_range(campaign_analytics_page):
...
def test_campaign_analytics_TC05_from_date_never_after_to_date(campaign_analytics_page):
...
@pytest.mark.regression
def test_campaign_analytics_TC06_future_date_range(campaign_analytics_page):
...
@pytest.mark.regression
def test_campaign_analytics_TC07_daily_report_type(campaign_analytics_page):
...
@pytest.mark.regression
def test_campaign_analytics_TC08_monthly_report_type(campaign_analytics_page):
...
@pytest.mark.regression
def test_campaign_analytics_TC09_group_by_option_changes_grouping(campaign_analytics_page):
...
@pytest.mark.regression
def test_campaign_analytics_TC10_search_valid_campaign(campaign_analytics_page):
    """TC_10: Searching by a valid Campaign name returns matching records."""
    ensure_on_report_page(campaign_analytics_page)
    if not campaign_analytics_page.has_records():
        pytest.skip("No records available to search")
    campaign_analytics_page.search("a")
    assert campaign_analytics_page.has_records() or campaign_analytics_page.has_no_records_message()
    campaign_analytics_page.clear_search()


@pytest.mark.negative
def test_campaign_analytics_TC11_search_invalid_campaign(campaign_analytics_page):
    """TC_11: Searching a non-existing Campaign name shows the no-records
    state."""
    ensure_on_report_page(campaign_analytics_page)
...
@pytest.mark.regression
def test_campaign_analytics_TC12_filter_by_product(campaign_analytics_page):
    ...
    ensure_on_report_page(campaign_analytics_page)
    campaign_analytics_page.select_product_filter("MARKETING")
    assert campaign_analytics_page.get_product_filter_value() == "MARKETING"
    campaign_analytics_page.page.wait_for_timeout(1000)
    assert campaign_analytics_page.has_records() or campaign_analytics_page.has_no_records_message()
...
def test_campaign_analytics_filters_panel_shows_all_fields(campaign_analytics_page):
    ...
    assert campaign_analytics_page.is_element_present(campaign_analytics_page.FILTER_USER_INPUT, timeout=5000)
...
@pytest.mark.regression
def test_campaign_analytics_TC13_export_csv(campaign_analytics_page):
...
@pytest.mark.regression
def test_campaign_analytics_TC14_export_button_available_after_filtering(campaign_analytics_page):
...
@pytest.mark.regression
def test_campaign_analytics_TC15_hide_specific_column(campaign_analytics_page):
...
@pytest.mark.regression
def test_campaign_analytics_TC16_re_enable_hidden_column(campaign_analytics_page):
...
@pytest.mark.regression
def test_campaign_analytics_TC17_pagination_changes_data(campaign_analytics_page):
    """TC_17: Navigating to the next page changes the displayed data
    without breaking the UI."""
    ensure_on_report_page(campaign_analytics_page)
    before = campaign_analytics_page.get_column_values("duration")
    if not before:
        pytest.skip("No rows to paginate through")
    campaign_analytics_page.click_next_page()
    after = campaign_analytics_page.get_column_values("duration")
...
@pytest.mark.regression
def test_campaign_analytics_TC18_load_performance(campaign_analytics_page):
...
@pytest.mark.regression
def test_campaign_analytics_product_column_values(campaign_analytics_page):
    """Product column has values for every row."""
    ensure_on_report_page(campaign_analytics_page)
    values = campaign_analytics_page.get_column_values("product")
    if not values:
...
@pytest.mark.regression
def test_campaign_analytics_waba_number_column_values(campaign_analytics_page):
    """WABA Number column has values for every row (CONFIRMED live rows
    showed the same value "919202511257" throughout)."""
    ensure_on_report_page(campaign_analytics_page)
    values = campaign_analytics_page.get_column_values("waba_number")
    if not values:
        pytest.skip("No rows to validate")
    assert all(v.strip() != "" for v in values)
...
@pytest.mark.regression
def test_campaign_analytics_campaign_column_values(campaign_analytics_page):
    """Campaign column has values for every row (CONFIRMED live DOM
    identifier column unique to this report)."""
    ensure_on_report_page(campaign_analytics_page)
    values = campaign_analytics_page.get_column_values("campaign")
    if not values:
        pytest.skip("No rows to validate")
    assert all(v.strip() != "" for v in values)


def test_campaign_analytics_template_column_values(campaign_analytics_page):
    """Template column has values for every row (CONFIRMED live DOM
    identifier column unique to this report)."""
    ensure_on_report_page(campaign_analytics_page)
    values = campaign_analytics_page.get_column_values("template")
    if not values:
        pytest.skip("No rows to validate")
    assert all(v.strip() != "" for v in values)
...
@pytest.mark.regression
def test_campaign_analytics_total_column_values(campaign_analytics_page):
    """Total column has values for every row."""
    ensure_on_report_page(campaign_analytics_page)
    values = campaign_analytics_page.get_column_values("total")
    if not values:
        pytest.skip("No rows to validate")
    assert all(v.strip() != "" for v in values)
...
@pytest.mark.regression
def test_campaign_analytics_read_column_values(campaign_analytics_page):
    """Read column has values for every row."""
    ensure_on_report_page(campaign_analytics_page)
    values = campaign_analytics_page.get_column_values("read")
    if not values:
        pytest.skip("No rows to validate")
    assert all(v.strip() != "" for v in values)
...
@pytest.mark.regression
def test_campaign_analytics_interactions_column_values(campaign_analytics_page):
...
@pytest.mark.regression
def test_campaign_analytics_sent_column_values(campaign_analytics_page):
    """Sent column has values for every row."""
    ensure_on_report_page(campaign_analytics_page)
    values = campaign_analytics_page.get_column_values("sent")
    if not values:
        pytest.skip("No rows to validate")
    assert all(v.strip() != "" for v in values)
...
@pytest.mark.regression
def test_campaign_analytics_delivered_column_values(campaign_analytics_page):
    """Delivered column has values for every row."""
    ensure_on_report_page(campaign_analytics_page)
    values = campaign_analytics_page.get_column_values("delivered")
    if not values:
        pytest.skip("No rows to validate")
    assert all(v.strip() != "" for v in values)
...
@pytest.mark.regression
def test_campaign_analytics_failed_column_values(campaign_analytics_page):
    """Failed column has values for every row."""
    ensure_on_report_page(campaign_analytics_page)
    values = campaign_analytics_page.get_column_values("failed")
    if not values:
        pytest.skip("No rows to validate")
    assert all(v.strip() != "" for v in values)
...
@pytest.mark.regression
def test_campaign_analytics_rejected_column_values(campaign_analytics_page):
    """Rejected column has values for every row."""
    ensure_on_report_page(campaign_analytics_page)
    values = campaign_analytics_page.get_column_values("rejected")
    if not values:
        pytest.skip("No rows to validate")
    assert all(v.strip() != "" for v in values)
...
@pytest.mark.regression
def test_campaign_analytics_dlr_awaited_column_values(campaign_analytics_page):
    """DLR Awaited column has values for every row."""
    ensure_on_report_page(campaign_analytics_page)
    values = campaign_analytics_page.get_column_values("dlr_awaited")
    if not values:
        pytest.skip("No rows to validate")
    assert all(v.strip() != "" for v in values)
