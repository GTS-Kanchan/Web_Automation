"""
WhatsApp Template Analytics — Single Sequential Flow
=====================================================
Covers: Channels → WhatsApp → Analytics → Template page
(URL: /whatsapp/analytics/template; breadcrumb reads "WhatsApp Template Analytics"
but this page's own <h1> reads plain "Template Analytics" — CONFIRMED live DOM
...
Key confirmed structural facts that shaped this suite:
  - 17 columns total: duration/product/waba-number/template (4 fixed
    identifiers) + the FULL 13-metric set.
  ...
  - Search placeholder is "Search by Template".
  - CONFIRMED live pagination total at capture time: 317 results.
...
`template_analytics_page` (built on conftest.py's
`module_logged_in_page`) to avoid shadowing pytest-playwright's reserved
`page` fixture.

Run:
    pytest tests/test_whatsapp_template_analytics_flow.py -v
"""
import pytest

from pages.whatsapp.whatsapp_template_analytics_page import WhatsappTemplateAnalyticsPage
...
def template_analytics_page(module_logged_in_page):
    p = WhatsappTemplateAnalyticsPage(module_logged_in_page)
...
def test_template_analytics_TC01_page_loads(template_analytics_page):
    """TC_01: WhatsApp Template Analytics page loads successfully without
    errors."""
    ensure_on_report_page(template_analytics_page)
    assert template_analytics_page.is_report_page(), "URL should contain /whatsapp/analytics/template"
...
def test_template_analytics_title_displayed(template_analytics_page):
    """Page title 'Template Analytics' is visible (CONFIRMED live DOM — this
    report's own <h1> omits the "WhatsApp" prefix seen in the breadcrumb)."""
    ensure_on_report_page(template_analytics_page)
    assert "Template Analytics" in template_analytics_page.get_page_title_text()
...
def test_template_analytics_TC03_columns_displayed(template_analytics_page):
    ...
    for expected in ['duration', 'product', 'waba number', 'template', 'total']:
...
def test_template_analytics_TC10_search_valid_template(template_analytics_page):
    """TC_10: Searching by a valid Template value returns matching
    records."""
    ensure_on_report_page(template_analytics_page)
    if not template_analytics_page.has_records():
        pytest.skip("No records available to search")
    template_analytics_page.search("a")
    assert template_analytics_page.has_records() or template_analytics_page.has_no_records_message()
...
def test_template_analytics_TC11_search_invalid_template(template_analytics_page):
    """TC_11: Searching a non-existing Template value shows the
    no-records state."""
    ensure_on_report_page(template_analytics_page)
...
def test_template_analytics_TC12_filter_by_product(template_analytics_page):
    ...
    ensure_on_report_page(template_analytics_page)
    template_analytics_page.select_product_filter("MARKETING")
    assert template_analytics_page.get_product_filter_value() == "MARKETING"
    template_analytics_page.page.wait_for_timeout(1000)
    assert template_analytics_page.has_records() or template_analytics_page.has_no_records_message()
...
def test_template_analytics_TC17_pagination_changes_data(template_analytics_page):
    ...
    ensure_on_report_page(template_analytics_page)
    before = template_analytics_page.get_column_values("duration")
    if not before:
        pytest.skip("No rows to paginate through")
    template_analytics_page.click_next_page()
    after = template_analytics_page.get_column_values("duration")
...
@pytest.mark.regression
def test_template_analytics_waba_number_column_values(template_analytics_page):
    """WABA Number column has values for every row (CONFIRMED live rows
    showed the same value "919202511257" throughout)."""
    ensure_on_report_page(template_analytics_page)
    values = template_analytics_page.get_column_values("waba_number")
    if not values:
        pytest.skip("No rows to validate")
...
@pytest.mark.regression
def test_template_analytics_template_column_values(template_analytics_page):
    """Template column has values for every row (CONFIRMED live DOM
    identifier column unique to this report)."""
    ensure_on_report_page(template_analytics_page)
    values = template_analytics_page.get_column_values("template")
    if not values:
        pytest.skip("No rows to validate")
    assert all(v.strip() != "" for v in values)
...
@pytest.mark.regression
def test_template_analytics_total_column_values(template_analytics_page):
    """Total column has values for every row."""
    ensure_on_report_page(template_analytics_page)
    values = template_analytics_page.get_column_values("total")
    if not values:
        pytest.skip("No rows to validate")
    assert all(v.strip() != "" for v in values)
...
@pytest.mark.regression
def test_template_analytics_read_column_values(template_analytics_page):
    """Read column has values for every row."""
    ensure_on_report_page(template_analytics_page)
    values = template_analytics_page.get_column_values("read")
    if not values:
        pytest.skip("No rows to validate")
    assert all(v.strip() != "" for v in values)
...
@pytest.mark.regression
def test_template_analytics_interactions_column_values(template_analytics_page):
...
@pytest.mark.regression
def test_template_analytics_sent_column_values(template_analytics_page):
    """Sent column has values for every row."""
    ensure_on_report_page(template_analytics_page)
    values = template_analytics_page.get_column_values("sent")
    if not values:
        pytest.skip("No rows to validate")
    assert all(v.strip() != "" for v in values)
...
@pytest.mark.regression
def test_template_analytics_delivered_column_values(template_analytics_page):
    """Delivered column has values for every row."""
    ensure_on_report_page(template_analytics_page)
    values = template_analytics_page.get_column_values("delivered")
    if not values:
        pytest.skip("No rows to validate")
    assert all(v.strip() != "" for v in values)
...
@pytest.mark.regression
def test_template_analytics_failed_column_values(template_analytics_page):
    """Failed column has values for every row."""
    ensure_on_report_page(template_analytics_page)
    values = template_analytics_page.get_column_values("failed")
    if not values:
        pytest.skip("No rows to validate")
    assert all(v.strip() != "" for v in values)
...
@pytest.mark.regression
def test_template_analytics_rejected_column_values(template_analytics_page):
    """Rejected column has values for every row."""
    ensure_on_report_page(template_analytics_page)
    values = template_analytics_page.get_column_values("rejected")
    if not values:
        pytest.skip("No rows to validate")
    assert all(v.strip() != "" for v in values)
...
@pytest.mark.regression
def test_template_analytics_dlr_awaited_column_values(template_analytics_page):
    """DLR Awaited column has values for every row."""
    ensure_on_report_page(template_analytics_page)
    values = template_analytics_page.get_column_values("dlr_awaited")
    if not values:
        pytest.skip("No rows to validate")
    assert all(v.strip() != "" for v in values)
