"""
WhatsApp Message Type Analytics — Single Sequential Flow
=====================================================
Covers: Channels → WhatsApp → Analytics → Message Type page
(URL: /whatsapp/analytics/message-type; breadcrumb reads "WhatsApp Message Type Analytics"
but this page's own <h1> reads plain "Message Type Analytics" — CONFIRMED live DOM
...
Key confirmed structural facts that shaped this suite:
  - 17 columns total: duration/product/waba-number/message-type (4
  ...
  - Search placeholder is "Search by Message Type".
  - CONFIRMED live pagination total at capture time: 170 results.
...
`message_type_analytics_page` (built on conftest.py's
`module_logged_in_page`) to avoid shadowing pytest-playwright's reserved
`page` fixture.

Run:
    pytest tests/test_whatsapp_message_type_analytics_flow.py -v
"""
import pytest

from pages.whatsapp.whatsapp_message_type_analytics_page import WhatsappMessageTypeAnalyticsPage
...
def message_type_analytics_page(module_logged_in_page):
    p = WhatsappMessageTypeAnalyticsPage(module_logged_in_page)
...
def test_message_type_analytics_TC01_page_loads(message_type_analytics_page):
    """TC_01: WhatsApp Message Type Analytics page loads successfully without
    errors."""
    ensure_on_report_page(message_type_analytics_page)
    assert message_type_analytics_page.is_report_page(), "URL should contain /whatsapp/analytics/message-type"
...
def test_message_type_analytics_title_displayed(message_type_analytics_page):
    """Page title 'Message Type Analytics' is visible (CONFIRMED live DOM — this
    report's own <h1> omits the "WhatsApp" prefix seen in the breadcrumb)."""
    ensure_on_report_page(message_type_analytics_page)
    assert "Message Type Analytics" in message_type_analytics_page.get_page_title_text()
...
def test_message_type_analytics_TC03_columns_displayed(message_type_analytics_page):
    ...
    for expected in ['duration', 'product', 'waba number', 'message type', 'total']:
...
def test_message_type_analytics_TC10_search_valid_message_type(message_type_analytics_page):
    """TC_10: Searching by a valid Message Type value returns matching
    records."""
    ensure_on_report_page(message_type_analytics_page)
    if not message_type_analytics_page.has_records():
        pytest.skip("No records available to search")
    message_type_analytics_page.search("a")
    assert message_type_analytics_page.has_records() or message_type_analytics_page.has_no_records_message()
...
def test_message_type_analytics_TC11_search_invalid_message_type(message_type_analytics_page):
    """TC_11: Searching a non-existing Message Type value shows the
    no-records state."""
    ensure_on_report_page(message_type_analytics_page)
...
def test_message_type_analytics_TC12_filter_by_product(message_type_analytics_page):
    ...
    ensure_on_report_page(message_type_analytics_page)
    message_type_analytics_page.select_product_filter("MARKETING")
    assert message_type_analytics_page.get_product_filter_value() == "MARKETING"
    message_type_analytics_page.page.wait_for_timeout(1000)
    assert message_type_analytics_page.has_records() or message_type_analytics_page.has_no_records_message()
...
def test_message_type_analytics_filter_by_user(message_type_analytics_page):
    ...
    docstring)."""
    ensure_on_report_page(message_type_analytics_page)
    message_type_analytics_page.filter_by_user("a")
    assert message_type_analytics_page.has_records() or message_type_analytics_page.has_no_records_message()
...
def test_message_type_analytics_table_loads(message_type_analytics_page):
    """Message Type analytics table loads with records displayed correctly."""
    ensure_on_report_page(message_type_analytics_page)
    assert message_type_analytics_page.has_records() or message_type_analytics_page.has_no_records_message()
...
@pytest.mark.regression
def test_message_type_analytics_waba_number_column_values(message_type_analytics_page):
    """WABA Number column has values for every row (CONFIRMED live rows
    showed the same value "919202511257" throughout)."""
    ensure_on_report_page(message_type_analytics_page)
    values = message_type_analytics_page.get_column_values("waba_number")
    if not values:
        pytest.skip("No rows to validate")
...
def test_message_type_analytics_message_type_column_values(message_type_analytics_page):
    """Message Type column has values for every row (CONFIRMED live DOM
...
@pytest.mark.regression
def test_message_type_analytics_total_column_values(message_type_analytics_page):
    """Total column has values for every row."""
    ensure_on_report_page(message_type_analytics_page)
    values = message_type_analytics_page.get_column_values("total")
    if not values:
        pytest.skip("No rows to validate")
    assert all(v.strip() != "" for v in values)
...
@pytest.mark.regression
def test_message_type_analytics_read_column_values(message_type_analytics_page):
    """Read column has values for every row."""
    ensure_on_report_page(message_type_analytics_page)
    values = message_type_analytics_page.get_column_values("read")
    if not values:
        pytest.skip("No rows to validate")
    assert all(v.strip() != "" for v in values)
...
@pytest.mark.regression
def test_message_type_analytics_interactions_column_values(message_type_analytics_page):
...
@pytest.mark.regression
def test_message_type_analytics_sent_column_values(message_type_analytics_page):
    """Sent column has values for every row."""
    ensure_on_report_page(message_type_analytics_page)
    values = message_type_analytics_page.get_column_values("sent")
    if not values:
        pytest.skip("No rows to validate")
    assert all(v.strip() != "" for v in values)
...
@pytest.mark.regression
def test_message_type_analytics_delivered_column_values(message_type_analytics_page):
    """Delivered column has values for every row."""
    ensure_on_report_page(message_type_analytics_page)
    values = message_type_analytics_page.get_column_values("delivered")
    if not values:
        pytest.skip("No rows to validate")
    assert all(v.strip() != "" for v in values)
...
@pytest.mark.regression
def test_message_type_analytics_failed_column_values(message_type_analytics_page):
    """Failed column has values for every row."""
    ensure_on_report_page(message_type_analytics_page)
    values = message_type_analytics_page.get_column_values("failed")
    if not values:
        pytest.skip("No rows to validate")
    assert all(v.strip() != "" for v in values)
...
@pytest.mark.regression
def test_message_type_analytics_rejected_column_values(message_type_analytics_page):
    """Rejected column has values for every row."""
    ensure_on_report_page(message_type_analytics_page)
    values = message_type_analytics_page.get_column_values("rejected")
    if not values:
        pytest.skip("No rows to validate")
    assert all(v.strip() != "" for v in values)
...
@pytest.mark.regression
def test_message_type_analytics_dlr_awaited_column_values(message_type_analytics_page):
    """DLR Awaited column has values for every row."""
    ensure_on_report_page(message_type_analytics_page)
    values = message_type_analytics_page.get_column_values("dlr_awaited")
    if not values:
        pytest.skip("No rows to validate")
    assert all(v.strip() != "" for v in values)
