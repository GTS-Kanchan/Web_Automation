...
# the project owner (the RCS Campaigns page's default on-screen table columns),
# not a guess and not the CSV/Excel export header list above (which is a
# separate, already-existing concept). See
# constants/rcs_campaign_ui_headers.py for the full header list and provenance note.
# ══════════════════════════════════════════════════════════════════════════════

@pytest.mark.smoke
def test_ui_default_table_headers_full(campaign_page):
    """All confirmed default on-screen table column headers for the RCS Campaigns
    page are present. Full-list check (every header in
    EXPECTED_RCS_CAMPAIGN_UI_HEADERS), following this suite's established
