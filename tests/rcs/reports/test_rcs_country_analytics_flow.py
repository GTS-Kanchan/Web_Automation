...
from constants.rcs_country_analytics_ui_headers import EXPECTED_RCS_COUNTRY_ANALYTICS_UI_HEADERS
...
def test_country_analytics_filter_by_user(country_analytics_page):
    ...
    ensure_on_report_page(country_analytics_page)
    country_analytics_page.filter_by_user("kanchan")
    assert country_analytics_page.has_records() or country_analytics_page.has_no_records_message()
...
# the project owner (the RCS Country Analytics page's default on-screen table columns),
# not a guess and not the CSV/Excel export header list above (which is a
# separate, already-existing concept). See
# constants/rcs_country_analytics_ui_headers.py for the full header list and provenance note.
# ══════════════════════════════════════════════════════════════════════════════

@pytest.mark.smoke
def test_ui_default_table_headers_full(country_analytics_page):
    """All confirmed default on-screen table column headers for the RCS Country Analytics
    page are present. Full-list check (every header in
    EXPECTED_RCS_COUNTRY_ANALYTICS_UI_HEADERS), following this suite's established
    case-insensitive substring-per-header convention."""
    country_analytics_page.navigate_to_report()
    headers = country_analytics_page.get_visible_column_headers()
    for col in EXPECTED_RCS_COUNTRY_ANALYTICS_UI_HEADERS:
