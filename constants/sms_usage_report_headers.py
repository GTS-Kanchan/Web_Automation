"""constants/sms_usage_report_headers.py — single source of truth for the
SMS Usage Report's expected CSV/Excel export header names, in the exact
order the downloaded file must contain them.

Do NOT duplicate this list inline in test files. Import
EXPECTED_SMS_USAGE_REPORT_HEADERS from here everywhere an SMS Usage Report
export file needs its headers validated:
    ...
    validate_file_headers(downloaded_file_path, EXPECTED_SMS_USAGE_REPORT_HEADERS)
...
confirming the change against the real Usage Report export first.
"""

EXPECTED_SMS_USAGE_REPORT_HEADERS = [
    "Duration",
    "Total Count",
    "Submitted Count",
    "Delivered Count",
