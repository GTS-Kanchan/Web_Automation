"""constants/sms_sender_report_headers.py — single source of truth for
the SMS Sender Report's expected CSV/Excel export header names, in the
exact order the downloaded file must contain them.

Do NOT duplicate this list inline in test files. Import
EXPECTED_SMS_SENDER_REPORT_HEADERS from here everywhere an SMS Sender
Report export file needs its headers validated:
    ...
    validate_file_headers(downloaded_file_path, EXPECTED_SMS_SENDER_REPORT_HEADERS)
...
confirming the change against the real Sender Report export first.
"""

EXPECTED_SMS_SENDER_REPORT_HEADERS = [
    "Duration",
    "Sender",
    "Product",
    "Total Count",
